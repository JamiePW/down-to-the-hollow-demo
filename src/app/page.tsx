          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                ...
                <div className="w-32">
                  <Progress value={(player.hp / effectiveStats.maxHp) * 100} className="h-3 bg-gray-700" />
                  <span className="text-xs text-gray-400">{player.hp}/{effectiveStats.maxHp}</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="w-24">
                  <Progress value={(player.pressure / effectiveStats.maxPressure) * 100} className="h-3 bg-gray-700" />
                  <span className="text-xs text-gray-400">{player.pressure}/{effectiveStats.maxPressure}</span>
                </div>
              </div>
            </div>
        ...
        <DialogContent className="bg-gray-800 border-gray-600">
          ...
          <DialogFooter className="flex flex-col gap-2">
            {dialogContent.choices?.map((choice, index) => (
              <Button key={index} onClick={choice.action} className="w-full">
                {choice.text}
              </Button>
            ))}
          </DialogFooter>
        ...
        <div className="max-w-6xl mx-auto flex justify-center gap-3">
          ...
          <Button 
            onClick={() => {
              setDialogContent({
                title: '📖 游戏说明',
                description: '这是对《绝区零》"走格子"玩法的还原。核心机制包括：1. 棋盘探索：点击相邻格子移动；2. 压力系统：每次移动增加压力，压力满时获得负面效果；3. 鸣徽系统：战斗和事件中获得增益道具；4. 战斗系统：简化版回合制战斗；5. 资源管理：金币用于商店购买鸣徽。目标：击败右下角的BOSS！'
              })
              setDialogOpen(true)
            }}
            variant="outline"
            className="gap-2 text-gray-300 border-gray-600 hover:bg-gray-700 hover:text-white"
          >
